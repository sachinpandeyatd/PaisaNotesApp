...
fun AccountWithBalanceTuple.toDomainModel() = Account(id, name, type, initialBalance, currentBalance, statementDay, dueDay)
fun AccountEntity.toDomainModel() = Account(id, name, type, initialBalance, initialBalance, statementDay, dueDay) // Fallback

fun Account.toEntity(syncStatus: SyncStatus = SyncStatus.PENDING_INSERT) = AccountEntity(
    id = id, name = name, type = type, initialBalance = initialBalance, statementDay = statementDay, dueDay = dueDay, syncStatus = syncStatus
)

fun AccountEntity.toDto(): AccountDto {
    val formatter = DateTimeFormatter.ISO_INSTANT.withZone(ZoneId.of("UTC"))
    return AccountDto(id, name, type, initialBalance, statementDay, dueDay, formatter.format(Instant.ofEpochMilli(createdAt)), formatter.format(Instant.ofEpochMilli(updatedAt)), isDeleted)
}
...
fun AccountDto.toEntity(): AccountEntity {
    ...
    return AccountEntity(id, name, type, initialBalance, statementDay, dueDay, parsedCreated, parsedUpdated, isDeleted, SyncStatus.SYNCED)
}